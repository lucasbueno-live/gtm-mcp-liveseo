...
type Schema$ContainerVersion = tagmanager_v2.Schema$ContainerVersion;

export const VERSION_ITEMS_PER_PAGE = 20;
...
export function processVersionData(
  ...
  page = 1,
  itemsPerPage = VERSION_ITEMS_PER_PAGE,
  includeSummary = true,
  ...
  const selected = resourceMap[resourceType] ?? [];
  const paginated = paginateArray(selected, page, itemsPerPage);

  const result: ProcessedVersionResponse = {
    version: baseVersion,
    [resourceType]: paginated.items,
  ...
  if (includeSummary) result.summary = summary;
  return result;
}
